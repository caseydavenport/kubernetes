...
The master requires the CA certificate, `ca.pem`; the `apiserver` certificate, `apiserver.pem` and its private key, `apiserver-key.pem` (generated by following the steps linked above). 
